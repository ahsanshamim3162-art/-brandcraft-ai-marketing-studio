...
import { campaignGenerationProvider } from "../lib/campaign-generator";
...
router.get("/campaigns", async (_req, res): Promise<void> => {
  const campaigns = await db
    .select({
      ...
      brandName: campaignsTable.brandName,
      industry: campaignsTable.industry,
...
router.get("/campaigns/summary", async (_req, res): Promise<void> => {
  ...
  res.json(
    GetCampaignSummaryResponse.parse({
      totalCampaigns: campaigns.length,
      thisMonth: thisMonth.length,
      latestCampaign: campaigns[0]?.name ?? null,
    }),
  );
});
...
router.post("/campaigns/generate", async (req, res): Promise<void> => {
  ...
  const strategy = campaignGenerationProvider.generate(parsed.data);
...
router.post("/campaigns", async (req, res): Promise<void> => {
  ...
  const [campaign] = await db
    ...
    .values({
      name: parsed.data.strategy.overview.campaignName,
      brandName: parsed.data.brief.brandName,
      industry: parsed.data.brief.industry,
      objective: parsed.data.brief.objective,
...
router.get("/campaigns/:id", async (req, res): Promise<void> => {
  ...
  if (!params.success) {
    res.status(400).json({ error: "Invalid campaign identifier." });
    return;
  }
...
router.patch("/campaigns/:id", async (req, res): Promise<void> => {
  ...
  if (!params.success || !body.success) {
    res.status(400).json({ error: "The campaign update is not valid." });
    return;
  }
  ...
  if (body.data.brief) {
    updates.brief = body.data.brief;
    updates.brandName = body.data.brief.brandName;
    updates.industry = body.data.brief.industry;
    updates.objective = body.data.brief.objective;
  }
  if (body.data.strategy) {
    updates.strategy = body.data.strategy;
    updates.name = body.data.strategy.overview.campaignName;
  }
